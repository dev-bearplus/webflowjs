const Script = () => {
    ...
    function activeTab() {
        if ($('[data-tab="wrap"]').length >= 1) {
            $('[data-tab="wrap"]').each((idx, el) => {
                let allTabs = $(el).find('[data-tab="head"]');
                allTabs.on('click', function(e) {
                    console.log($(this))
                    e.preventDefault();
                    let target = $(this).index();
                    let parent = $(this).closest('[data-tab="wrap"]')
                    parent.find('[data-tab="head"]').removeClass('active');
                    parent.find('[data-tab="content"]').removeClass('active');
                    $(this).addClass('active')
                    parent.find('[data-tab="content"]').eq(target).addClass('active');
                })
            })
        }
    }
    ...
    function scriptAllPage() {
        ...
        scriptService();
        function scriptFintech() {
            if ($('.fintechpage').length) {
                // Welcome to...
                console.log("Welcome to Fintech page");
                function fintechFaq() {
        ...
        function scriptBlogs() {
            if ($('.blogspage').length) {
                ...
                if ($('.sccontentblogs-form-input').length) { 
                    let allDropItems = $('.sccontentblogs-form-drop-item');
                    $('.sccontentblogs-form-input').on('keyup', function (e) {
                        if ($(this).val() !== '') {
                            $('.sccontentblogs-form-drop').addClass('active');
                        } else {                    
                            $('.sccontentblogs-form-drop').removeClass('active');
                        }
                        let inputVal = $(this).val().toLowerCase().replaceAll(' ', '');
                        if (inputVal !== '') {
                            allDropItems.each(function (index, item) {
                                let textVal = $(item).find('.drop-post-item-title .heading.h6').text().toLowerCase().replaceAll(' ', '');
                                if (textVal.includes(inputVal)) {
                                    $(item).addClass('on-show');
                                } else {
                                    $(item).removeClass('on-show');
                                }
                            })
                        } else {
                            allDropItems.addClass('on-show');
                        }
                        if ($('.sccontentblogs-form-drop-item.on-show').length === 0) {
                            $('.sccontentblogs-form-empty').addClass('on-empty');
                        } else {
                            $('.sccontentblogs-form-empty').removeClass('on-empty');
                        }
                    })
                    $('.sccontentblogs-form-input').on('focus', function (e) {
                        $('.sccontentblogs-form-drop').addClass('active');
                    })
                    $('.sccontentblogs-form-input').on('blur', function (e) {
                        if ($('.sccontentblogs-form-drop:hover').length) {
                            $('.sccontentblogs-form-drop').addClass('active');
                        } else {                    
                            $('.sccontentblogs-form-drop').removeClass('active');
                        }
                    })
                    $('.sccontentblogs-form-drop').on('mouseleave', function (e) {
                        scrollmain.start()
                    })
                    $('.sccontentblogs-form-drop').on('mouseenter', function (e) {                  
                        scrollmain.stop()
                    })
                }
            }
        ...
        function scriptCto() {
            ...
            if ($('.ctopage').length) {
                setTimeout(() => {
                    ...
                    ScrollTrigger.create({
                        trigger: $('.scservicehome.idm').eq(0),
                        start: 'top bottom',
                        onEnter: () => {
                            console.log('init')
                            scrollmain.update()
