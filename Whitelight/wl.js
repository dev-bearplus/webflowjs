const mainScript = () => {
            ...
            switch (type) {
                ...
                case 'ext':
                    ...
                    if (targetElExt.attr('data-cursor-ext')) {
                        if (targetElExt.attr('data-share') == 'copy') {
                            if (targetElExt.hasClass('copied')) {
                                $('.cursor-txt-wrap').find('.cursor-inner-txt').text('Copied!')
                            } else {
    ...
    class CardtllHeroAnimate {
        ...
        setup() {
            ...
            allBtns.each((idx, el) => {
                if (viewport.w > 991 && $(el).hasClass('hidden-dk')) {return}
                let btnTxt;
                let hasText = $(el).find('.btn .txt-btn').length >= 1
                if (hasText) {
                    btnTxt = new SplitType($(el).find('.btn .txt-btn').get(0), {types: 'lines, words', lineClass: 'bp-line'})
                } else {
                    btnTxt = $(el).find('.ic-btn')
                }
                this.tlTitle
                .from(hasText ? btnTxt.words : btnTxt, {autoAlpha: 0, yPercent: hasText ? 100 : 60, duration: .4, stagger: .04}, `<=${idx * .014}`)
                if ($(el).find('.btn-bg').length >= 1) {
                    this.tlTitle
    ...
    class CardtlMainAnimate {
        ...
        setup() {
            ...
            this.tlActs
            let allBtns = $('.cardtl-main-acts .btn-stage')
            allBtns.each((idx, el) => {
                let btnTxt;
                let hasText = $(el).find('.btn .txt-btn').length >= 1
                if (hasText) {
                    btnTxt = new SplitType('.cardtl-main-acts .btn .txt-btn', {types: 'lines, words', lineClass: 'bp-line'})
                } else {
                    btnTxt = $(el).find('.ic-btn')
                }
                this.tlActs
        ...
        interaction() {
            function shareBtnTxt(el) {
                if ($(el).hasClass('copied')) {return}
                let url = window.location.href;
                copyTextToClipboard(url)
                $(el).addClass('copied')
                let orginText = $(el).find('.txt').text();
                let orginWidth = $(el).find('.txt').width();
                gsap.to($(el).find('.txt'), {autoAlpha: 0, duration: .3, onComplete: () => {
                    $(el).find('.txt').text('Copied!')
                    let newWidth = $(el).find('.txt').width();
                    gsap.set($(el).find('.txt'), {width: orginWidth, autoAlpha: 1})
                    gsap.to($(el).find('.txt'), {autoAlpha: 1, width: newWidth, duration: .3, clearProps: 'width', onComplete: () => {
                        setTimeout(() => {
                            gsap.set($(el).find('.txt'), {width: newWidth})
                            gsap.to($(el).find('.txt'), {autoAlpha: 0, duration: .3, onComplete: () => {
                                $(el).find('.txt').text(orginText)
                                gsap.to($(el).find('.txt'), {width: orginWidth, autoAlpha: 1, duration: .3, clearProps: 'all'})
            ...
            $('[data-apply="share"]').on('click', function(e) {
                ...
                let url = window.location.href;
                if (type == 'copy') {
                    if ($(this).hasClass('cardtl-txt-link-wrap') && viewport.w < 991) {
                        shareBtnTxt(this)
                    } else if ($(this).hasClass('cardtl-hero-acts-btn-share') && viewport.w < 767) {
                        copyTextToClipboard(url)
                    } else {
            ...
            $('[data-share="copy"]').on('mouseleave', function(e) {
                $(this).removeClass('copied')
            })
