...
refreshOnBreakpoint();
  ...
  function updateCopyYear() {
    if ($('[data-copy="year"]').length >= 1) {
      let currYear = new Date().getFullYear();
      $('[data-copy="year"]').text(currYear)
    }
  }
      ...
      function homeSet() {
        ...
        if ($(window).width() > 991) {
            ...
            const swiperTestiName = new Swiper(".schome-testi-name-swiper", {
              ...
              on: {
                slideNextTransitionStart() {
                  ...
                  swiperTestiAvatar.slideTo(swiperTestiName.activeIndex);
                  swiperTestiContent.slideTo(swiperTestiName.activeIndex);
                  // if (curSc == 4) {
                  //   scrolled = true;
          ...
          const swiperTestiName = new Swiper(".schome-testi-name-swiper", {
            ...
            allowTouchMove: false,
            navigation: {
              nextEl: '.schome-testi-nav-btn.nav-bot',
              prevEl: '.schome-testi-nav-btn.nav-top',
            },
            on: {
              slideNextTransitionStart() {
                // swiperTestiAvatar.slideNext();
